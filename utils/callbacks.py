...
def load_callbacks(description='exp',results_folder='results',stages=12,shots=4,transpose='unet',prior='hssp',mode='sthocastic',model=None,dataset_path='image_val'):
    ...
    experiment = description + '_stage_'+str(stages)+'_shots_'+str(shots)+'_transpose_'+transpose+'_prior_'+prior+'_mode_'+mode
    ...
    try:
        os.mkdir(path+'validation_images')
    ...
    for i in [82]:
        ...
        path_i = path+'training_images/image_'+str(i)
        images_train_cb =tf.keras.callbacks.LambdaCallback(on_epoch_end=lambda epoch, logs: log_xnca(epoch, logs, model = model, val_img = train_img, fw_results = fw_train, name = "train_"+str(i),path=path_i))
      
        #train_img_2 = train_img_2.astype(np.float32)# / (2 ** 16 - 1)
        #train_img_2 = hf['cube'][147]
        
        #train_img_2 = tf.image.central_crop(train_img_2,0.25)[...,3:28]
        #train_img_2 = tf.expand_dims(train_img_2/np.max([np.max(train_img_2),1e-6]),0)
    ...
    for i in [1]:
        ...
        path_i = path+'validation_images/image_'+str(i)
        images_val_cb = tf.keras.callbacks.LambdaCallback(on_epoch_end=lambda epoch, logs: log_xnca(epoch, logs, model = model, val_img = val_img, fw_results = fw_val, name = "val_"+str(i),path=path_i))
    images_test_cb = []
    for i in [12]:
        ...
        path_i = path+'test_images/image_'+str(i)
            
        images_test_cb = tf.keras.callbacks.LambdaCallback(on_epoch_end=lambda epoch, logs: log_xnca(epoch, logs, model = model, val_img = test_img, fw_results = fw_test, name = "test_"+str(i),path=path_i))
...
def log_xnca(epoch, logs, model, val_img, fw_results, name,path):
    ...
    sio.savemat(path,{'Xgt': np.array(xgt), 'Xr': np.array(Xt[-1])})
    # Log the results images as an image summary.
