...
def customGaussianFunc(snr, input_shape):
    def add_gaussian_noise(x, divisor):      
                
        stddev = tf.math.sqrt(tf.math.divide(tf.math.pow(tf.norm(x, 'fro', axis= [0, 1], keepdims = True),2), divisor))
        return x + tf.random.normal(shape=x.shape,mean=0,stddev=stddev, dtype=x.dtype)
    snr = snr
    input_shape = input_shape
    m = input_shape[1]*input_shape[2]*input_shape[0]
    divisor = m*10**(snr/10)


    return lambda x: add_gaussian_noise(x, divisor = divisor)
...
def Input_image(image):
    ...
    function_gaussian_noise = customGaussianFunc(snr = 0, input_shape = images.shape)
    images=function_gaussian_noise(images)
    images = np.abs(images/np.max(images))############
...
class DataGenerator(Sequence):
    ...
    def __data_generation(self, images_names):
        ...
        for i, file_name in enumerate(images_names):
            ...
            X[i,] =tf.keras.layers.GaussianNoise(0.01, seed=None)(X[i,])
            X[i,] = X[i,]/tf.reduce_max(X[i,])
...
def Build_data_set_train(IMG_WIDTH=128, IMG_HEIGHT=128, IMG_WIDTH_o=128, IMG_HEIGHT_o=128, L_bands=31, L_imput=12, BATCH_SIZE=4, PATH=None):
    # Random split
