...
class DOE(Layer):
    ...
    def __init__(self, Mdoe=64, Mesce=64, wave_lengths=None, DOE_type='New', Trai=True, **kwargs):
        ...
        if DOE_type == 'New' or DOE_type == 'Zeros':
            self.Trai = Trai #Trai
            self.zernike_volume = loadmat('/home/hdsp-deep/Salmanwork_Sergio/Three_shots_Current/Base_zernike_64x64_nopadd.mat').get('HmBase').astype(np.float32)
        else:
            Hm_DOE = loadmat('/home/hdsp-deep/Salmanwork_Sergio/Three_shots_Current/Spiral_64x64_nopadd.mat').get('Hm').astype(np.float32)
            self.Hm_DOE = Hm_DOE
            
            self.Trai = False
        self.P = loadmat('/home/hdsp-deep/Salmanwork_Sergio/Three_shots_Current/Spiral_64x64_nopadd.mat').get('P').astype(np.float32)
